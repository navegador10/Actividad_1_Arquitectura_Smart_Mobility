...
- **Sensor Ambiental:** Sensor que monitorea condiciones climáticas y ambientales (temperatura, humedad, calidad del aire).
...
**Propósito:** Articular las interacciones principales entre los actores y el sistema, mostrando cómo se satisfacen los requisitos funcionales y no funcionales en escenarios reales de operación. Esta vista permite validar el comportamiento esperado del sistema desde la perspectiva de usuarios, operadores y sistemas externos (Kruchten, 1995, p. 60; módulo, p. 99).

```mermaid
flowchart LR
    U[Usuario Conductor/Peatón]
    C[Centro de Control]
    S[Sistema de Sensores IoT]
    E[Servicios Externos]
    A[App Móvil]

    subgraph Sistema["Sistema Inteligente de Movilidad Urbana"]
        UC1[UC01: Monitorear tráfico en tiempo real]
        UC2[UC02: Calcular ruta óptima]
        UC3[UC03: Gestionar semáforos inteligentes]
        UC4[UC04: Generar alerta de congestión o incidente]
        UC5[UC05: Consultar estado de vías e intersecciones]
        UC6[UC06: Administrar sensores y dispositivos IoT]
        UC7[UC07: Integrar datos meteorológicos externos]
    end
...
**UC04: Generar alerta de congestión o incidente**
1. Un sensor IoT o una cámara detecta una anomalía, como una reducción de velocidad por debajo del umbral o un incidente visual.
2. El gateway IoT publica el evento en el bus de mensajes del sistema.
3. El procesador de eventos valida y enriquece la alerta con contexto geoespacial y temporal.
4. El motor de análisis clasifica la severidad como media, alta o crítica.
5. El servicio de alertas notifica al Centro de Control y a los usuarios afectados a través de la App Móvil.
6. El sistema registra la alerta en la base de datos para fines de auditoría y análisis posterior.

**Atributos de calidad involucrados:** Rendimiento (procesamiento en menos de 2 segundos), disponibilidad (operación continua 24/7/365) y seguridad (transmisión cifrada y control de acceso).
